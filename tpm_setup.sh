...
# Bump this on every change to this script (including generated shell-
# integration content in Phase 5 below) so --status can tell a user their
# installed shell integration was generated by an older version and hint at
# --reinstall-scripts. Persisted into $STATE_FILE at the end of a successful
# run; never bump this mid-feature, only once per shipped change.
TPM_SETUP_VERSION="1.0.0"
...
print_usage() {
    ...
    printf "%s\n" "Version $TPM_SETUP_VERSION."
    printf "\n"
    printf "Usage: %s [--env-file <path>] [--uninstall] [--status] [--reinstall-scripts]\n\n" "$0"
    ...
    printf "%s\n" "                     changes."
    printf "%s\n" "  --reinstall-scripts"
    printf "%s\n" "                     Regenerate the unlock_tpm shell integration (bash/sh/"
    printf "%s\n" "                     tcsh startup hooks) from this script's current version,"
    printf "%s\n" "                     without touching any sealed TPM secrets. Use this after"
    printf "%s\n" "                     updating tpm_setup.sh to pick up shell-integration fixes"
    printf "%s\n" "                     -- --status will tell you when this is needed."
}
...
if [ "$STATUS" -eq 1 ]; then
    ...
    # chosen) -- both look like an empty string otherwise. Same idea for
    # SCRIPT_VERSION: empty means an install from before version tracking
    # existed, not literally version "".
    ...
    # Only nudge toward --reinstall-scripts when something is actually
    # installed here (STATE_FILE with real content) -- a fresh, never-set-up
    # machine has nothing to reinstall, and normal setup already covers it.
    if [ -f "$STATE_FILE" ]; then
        if [ -z "$SCRIPT_VERSION" ]; then
            printf "Installed shell integration: unknown version (predates version tracking)\n"
            printf "[TPM] Hint: run '%s --reinstall-scripts' to refresh it to v%s -- keeps existing secrets, no TPM changes.\n" "$0" "$TPM_SETUP_VERSION"
        elif [ "$SCRIPT_VERSION" != "$TPM_SETUP_VERSION" ]; then
            printf "Installed shell integration: v%s (this script is v%s)\n" "$SCRIPT_VERSION" "$TPM_SETUP_VERSION"
            printf "[TPM] Hint: run '%s --reinstall-scripts' to refresh it to v%s -- keeps existing secrets, no TPM changes.\n" "$0" "$TPM_SETUP_VERSION"
        else
            printf "Installed shell integration: v%s (up to date)\n" "$SCRIPT_VERSION"
        fi
    fi
...
# --reinstall-scripts only needs Phase 5 (shell integration) below, which
# needs no TPM access and no package/group/kernel-module setup -- skip
# straight past all of that (and the RESEED/Unlock-Strategy prompts) into
# an equivalent, minimal setup in the else branch further down.
if [ "$REINSTALL_SCRIPTS" -eq 0 ]; then
...
[ "$STRATEGY_CHOICE" != "2" ] && STRATEGY_CHOICE="1"
    ...
    STATE_FILE="$HOME/.tpm_keys_state"
    if [ ! -f "$STATE_FILE" ]; then
        printf "[TPM] ERROR: No existing installation found (%s is missing). Run %s with no arguments first to seed a secret before using --reinstall-scripts.\n" "$STATE_FILE" "$0" >&2
        exit 1
    fi
    API_AUTH_MODE="master"
    SSH_AGENT_AUTOSTART="no"
    ...
    if [ -z "$STRATEGY_CHOICE" ]; then
        ...
        # --reinstall-scripts asks nothing at all in the common case.
        printf "\n%s\n" "--- Unlock Strategy ---"
        printf "%s\n" "[1] Automatic : Prompt for PIN automatically when opening a new terminal."
        printf "%s\n" "[2] Manual    : Print a hint in new terminals, wait for you to run 'unlock_tpm'."
        printf "Choose (1 or 2) [default: 1]: "
    ...
    printf "\n%s\n" "=== Reinstalling shell integration (v$TPM_SETUP_VERSION) ==="
    printf "%s\n" "Skipping TPM secret entry/seeding; only regenerating shell startup files."
fi # REINSTALL_SCRIPTS
...
# Persists API_AUTH_MODE/SSH_AGENT_AUTOSTART/SECRET_NAMES (all either just
# freshly seeded above, or reused unchanged from a prior run) plus
# STRATEGY_CHOICE and this script's version, so the next re-run -- whether
# a normal RESEED=0 keep-existing run or --reinstall-scripts -- can
# regenerate Phase 5 below without re-asking, and --status can tell when
# the installed shell integration is older than this script.
printf 'API_AUTH_MODE=%s\nSSH_AGENT_AUTOSTART=%s\nSTRATEGY_CHOICE=%s\nSCRIPT_VERSION=%s\nSECRET_NAMES="%s"\n' \
    "$API_AUTH_MODE" "$SSH_AGENT_AUTOSTART" "$STRATEGY_CHOICE" "$TPM_SETUP_VERSION" "$SECRET_NAMES" \
...
if [ "$REINSTALL_SCRIPTS" -eq 1 ]; then
    printf "\n%s\n" "=== Reinstall Complete! ==="
    printf "Shell integration refreshed to v%s. Open a new terminal (or re-source your\n" "$TPM_SETUP_VERSION"
    printf "shell startup file) to pick up the change. Your sealed TPM secrets were not touched.\n"
else
    printf "\n%s\n" "=== Setup Complete! ==="
