...
function unlock_tpm {
    ...
    # Match the SPECIFIC key this install manages, not just "any ED25519
    # identity" -- found in the wild (dell-7550) that a stale, unrelated
    # ED25519 key already loaded in the agent (e.g. left over from before
    # this key was regenerated) was mistaken for "already loaded," so the
    # actual TPM-sealed key never got loaded at all.
    `$sshPubPath = '$sshKeyPath.pub'
    if (Test-Path `$sshPubPath) {
        `$expectedPub = (Get-Content `$sshPubPath -Raw).Trim()
        `$loadedPubs = & ssh-add -L 2>`$null
        if (-not (((`$loadedPubs -join "`n")) -match [regex]::Escape(`$expectedPub))) { `$needsSsh = `$true }
    } else {
...
"@
    ...
    # The closing marker is matched as ANY run of dashes (not a literal
    # fixed-length string) -- found in the wild (dell-7550) that an older
    # script version's already-installed block used a different dash count
    # than this version writes, which made a literal-length match silently
    # fail to find/replace it, appending a duplicate block instead.
    $blockPattern = '(?s)\r?\n?# --- TPM Secure Environment Setup \(PowerShell\) ---.*?# -{10,}\r?\n?'
